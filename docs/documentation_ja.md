...
`Tidy` オプション（`tidy: on`，CLI では `--tidy`）を有効にすると，空間効率を高めた描画モードになります．次の2つの仕組みを組み合わせています：

- **輪郭ベースの圧縮**：隣り合う部分木の輪郭の間に余白がある場合，ラベル間の最小間隔を保ちながら互いに引き寄せます．全体の幅は平均して3割ほど小さくなります．
- **動的コネクタ高さ**：ノードから子への縦の落差を，そのレベルでの子の広がりに応じて変化させます．ルート付近では枝が平たく，リーフ付近では急峻になりがちですが，このモードでは樹全体で枝の角度が揃います．同じ深さのノードの縦位置は揃ったままです．

程度を調整するためのオプションが2つあります（いずれも `Tidy` がオンのときのみ有効です）：

- `Tidy spacing`（`tidy_spacing`，デフォルト 1.0，目安 0.5〜3.0）：隣り合う部分木の間に保つ最小間隔の倍率です．大きくするとラベル間に余裕が生まれ，小さくするとより強く圧縮されます．
- `Tidy slope`（`tidy_slope`，デフォルト 0.3，目安 0.1〜1.0）：枝の広がりに対するコネクタ高さの増え方（傾き）を制御します．コネクタ高さの上限も slope に連動するため，この値を下げるだけで樹全体の高さも抑えられます．高さをほとんど増やさずに圧縮だけ効かせたい場合は `0.15` がおすすめです．

Tidy レイアウトがラベルの重なった図を出力することはありません（圧縮によって衝突が生じる場合は，直前の安全な配置に戻します）．上から下・左から右のどちらのレイアウトでも機能し，`Mirror` とも併用できます．なお `Tidy` がオンのときは `Radical symmetrization` オプションは無視されます．ギャラリーの [Multilingual カテゴリの例](https://yohasebe.github.io/rsyntaxtree/examples)はすべて `Tidy` を有効にして描画しています．
