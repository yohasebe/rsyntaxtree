...
Tidy レイアウトがラベルの重なった図を出力することはありません（圧縮によって衝突が生じる場合は，直前の安全な配置に戻します）．上から下・左から右のどちらのレイアウトでも機能し，`Mirror` とも併用できます．なお Tidy が有効なときは `Radical symmetrization` オプションは無視されます．[ギャラリー](https://yohasebe.github.io/rsyntaxtree/examples)の大半の図は `medium` で，X-bar 系と Morphology の例は `high` で描画しています．
