...
樹形図のすべてのブランチ（枝）やリーフ（葉）は，ノード（節点）に属します．ノードを作成するには，ラベルテキストを開始括弧の直後に配置します．ブランチは，空白で区切っていくつでも設定できます．（空白を含むノード・ラベルは，`<>` 記号を使って作成することができます．例えば， `Modal<>Auxは` ， `Modal Aux` と表示されます）．
...
ノードを表すテキストやリーフを表すテキストの中で改行を行たいたい場合，改行文字 `\n` を用いることができます．
...
**注意：** 単なる改行 `↩️` はスペースとして扱われます．`↩️` を1つ以上連続させた場合も1つのスペースとして扱われます．なお，ここでいう「スペース」とは，入力テキスト中の要素間の切れ目を表すスペースであり，空白文字として描画されるスペースでないことに注意してください．テキスト中でスペースを表示したいときには `<>` を使用します．
