...
ハイフンは下線の開始と終了を表すため，文字としてのハイフンは `\-` と書きます．HPSG やその周辺の枠組みでは HEAD-DTR や RELIED-ON のように素性名にハイフンが頻出し，そのすべてをエスケープするのは，その分野で使われない記法のために払う代償としては大きすぎます．`Hyphen`（`hyphen`，CLI では `--hyphen`）で2つの読み方を入れ替えられます．`literal` を指定すると，素のハイフンは文字として扱われ，下線は `\-下線\-` と書きます．
